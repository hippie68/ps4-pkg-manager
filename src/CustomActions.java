...
public class CustomActions {
	...
	public CustomActions(Shell parent) {
		...
		info.setText(
			"""
				Custom Actions are user-defined context menu entries that can be used to run platform-dependent commands on selected PKGs. The commands can contain PREFIXES and %variables%, which are evaluated in the following order:

				   CONFIRM -> A dialog asks the user for confirmation before running the command.
				   %dir% -> The first PKG file's directory.
				   %app%, %patch%, and %dlc% -> The next PKG file that is of the specified type.
				   %file% -> The next PKG file.
				   %files% -> All remaining PKG files, delimited by space characters.
				""");
	...
	public static boolean isValidPkgSelection(CustomAction action, PS4PKG[] pkgs) {
		...
		if (apps < requiredApps || patches < requiredPatches || dlcs < requiredDlcs)
			return false;
		else if (apps == requiredApps && patches == requiredPatches && dlcs == requiredDlcs && files == requiredFiles) {
	...
	private void addCustomAction(Composite parent, String name, String command) {
		...
		if (parent.getChildren().length == 2) {
			...
			nameHeader.setLocation(nameHeaderX, nameHeader.getLocation().y + headerCompositeSize.y - nameHeaderSize.y);
