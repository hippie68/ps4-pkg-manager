...
public class GUI {
	...
	private void createMenu() {
		...
		MenuItem mntmAddFtpFiles = new MenuItem(menuFile, SWT.NONE);
		mntmAddFtpFiles.setText("Add FT&P Files...");
		mntmAddFtpFiles.addListener(SWT.Selection, e -> new AddFtpFilesDialog(shell, this));
				...
				display.syncExec(() -> {
					if (isProcessingData(false) == true) {
						if (progressIndicator.getText().isEmpty())
							setProgressStatus(PROGRESS_TEXT);
					} else if (!progressIndicator.getText().isEmpty())
						setProgressStatus("");
				});
