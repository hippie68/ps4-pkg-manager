...
public class GUI {
    ...
    public void open() {
        ...
        shell = new Shell();
        shell.setText("PS4 PKG Manager");
        shell.setLayout(new BorderLayout());
        shell.addListener(SWT.Close, e -> {
            new ExitDialog(shell);
            saveGUIState();
    ...
    private void createMenu() {
        ...
        mntmAddDirectory.setText("Add " + Platform.DIRECTORY_TERM + "...");
        mntmAddDirectory.addListener(SWT.Selection, e -> {
            ...
            if (directory != null) {
                Set<String> pkgFiles = WatcherThread.getPkgFiles(directory);
                if (pkgFiles != null)
                    getCurrentTabContent().addFiles(pkgFiles.toArray(new String[0]));
            }
        });
        ...
        mntmAddDirectoryRecursively.addListener(SWT.Selection, e -> {
            ...
            String directory = dialog.open();
            if (directory != null) {
                Set<String> dirs = WatcherThread.getDirectoriesRecursively(directory);
                if (dirs != null)
                    for (String dir : dirs) {
                        Set<String> pkgFiles = WatcherThread.getPkgFiles(dir);
                        if (pkgFiles != null)
                            getCurrentTabContent().addFiles(pkgFiles.toArray(new String[0]));
                    }
            }
    ...
    private void createStatusBar() {
        ...
        currentSelectionStatus.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
        this.currentStatus = new Label(statusBar, SWT.NONE);
        currentStatus.setLayoutData(new GridData(SWT.NONE));
    }
    ...
    /** Puts text in the bottom status bar. */
    public void setCurrentStatus(String text) {
        this.currentStatus.setText(text);
    }
    ...
    private int loadGUIState() {
        ...
        // Load synchronized directories (the watcher thread must already be running).
        try {
            TabContent[] tabContents = getTabContents();
            for (int i = 0; i < tabContents.length; i++) {
                ...
                // Watch all loaded synchronized directories and add all PKG files.
                tabContents[i].watcherThread.generateWatchKeys();
                tabContents[i].watcherThread.addPkgFilesFromWatchedDirectories();
    ...
    // TODO: put a message about how to change the tab's name into the status bar.
    ...
    private void removeTab(TabItem item) {
        ...
        if (((TabContent) item.getControl()).getTable().getItemCount() != 0) {
            ...
            messageBox.setText("Close Tab");
            messageBox
                .setMessage("Do you really want to close the tab \"" + item.getText() + "\"?\nThis can't be undone.");
            int reply = messageBox.open();
            if (reply == SWT.NO)
                return;
        }
